...
- After each placement the CG is recalculated
...
class CargoEnv(gym.Env):
    ...
    def __init__(self, manifest_dir="data/manifests"):
        ...
        self.zones_template = self.ref.cargo_zones()  # dict of CargoZone
        ...
        # one integer from 0 to n_zones-1
        ...
        self.cargo_items     = []     # list of items to place this episode
        self.current_item_idx = 0     # which item we are placing right now
        self.zone_weights    = {}     # current cargo weight in each zone
        self.current_cg      = None  # current CG % MAC
    ...
    def reset(self, seed=None, options=None):
        ...
        # compute initial CG (empty aircraft + no cargo)
        self.current_cg = self.ref.x_to_percent_mac(
    ...
    def step(self, action):
        ...
        # ── recalculate CG ─────────────────────────────────────────────────
        self.current_cg = self._compute_cg()
        ...
        if done and not zone_overloaded:
            if self.cg_fwd <= self.current_cg <= self.cg_aft:
                reward += 10.0   # big bonus for completing within envelope
                # extra bonus for being close to target
    ...
    def _compute_cg(self):
        """
        Calculates current loaded CG in % MAC.
        Uses the same moment formula as Person 1's physics engine.
        """
        total_moment = (
            self.empty_aircraft.operating_empty_weight_kg
            * self.empty_aircraft.empty_cg_x_m
        )
        total_weight = self.empty_aircraft.operating_empty_weight_kg

        for zone_id, cargo_kg in self.zone_weights.items():
            if cargo_kg > 0:
                zone = self.zones_template[zone_id]
        ...
        return self.ref.x_to_percent_mac(cg_arm)
